...
if __name__ == "__main__":
    ...
    parser.add_argument('--lam', type=float, default=None, help="balanced hyperparameter: lambda")
    ...
    parser.add_argument('--beta', type=float, default=0, help="balanced hyperparameter: ascent rate of lambda")
    ...
    parser.add_argument("--finetuning", action="store_true", help="whether the mitigation method should be used fro finetuning")
    ...
    train_dataset, val_dataset, test_dataset, dataset_info = datasets.datasetPreprocessing(path="../dataset/", dataset_name=args.dataset, split_list=args.split_list, seed=args.seed, sensitive_attribute="sex", verbose=False)
    # print(dataset_info)
    args.dataset_info = dataset_info
    print(args.dataset_info)

    if args.lam is not None:
        print(f"Training with lambda={args.lam} and method={args.method}...")
        model, avg_losses, avg_fair_losses = train.train_model(args, train_dataset, val_dataset, args.lam)
        ...
        if args.fairness_metric_goal == "DP":
            ...
            print(f"Demographic Parity Difference: {fairness_metric:.4f}")
            ...
            print(f"Equalized Odds Difference: {fairness_metric:.4f}")
        ...
        if args.method == 'ERM' or args.method is None:
            ...
            print(f'Training with CvM regularizer with {args.fairness_metric_goal} goal...')
            if args.fairness_metric_goal == 'DP':
                lambdas = [0.0, 0.1, 0.5, 1.0, 2.0, 3.0, 5.0, 7.0, 10.0]
                # lambdas = [0.001, 0.01]
            else:
        ...
        # X_train, X_val, X_test, y_train, y_val, y_test, s_train, s_val, s_test, dataset_info = datasets.datasetPreprocessing("../dataset/", dataset_name=args.dataset, split_list=args.split_list, seed=args.seed, sensitive_attribute="sex", verbose=False)
        ...
        for lam, acc, fm, cvm_value in zip(lambdas, accuracies, fairness_metrics, CvMs):
            print(f"Lambda: {lam:.4f}, Accuracy: {acc:.4f}, Fairness Metric ({args.fairness_metric_goal}): {fm:.4f}, CvM: {cvm_value:.4f}")
        print('All Done!')
